...
from priorCVAE.losses import kl_divergence, scaled_sum_squared_loss, mean_squared_loss, square_maximum_mean_discrepancy, pixel_sum_loss
from priorCVAE.priors import SquaredExponential


def test_kl_divergence(num_data, dimension):
    """Test KL divergence between a Gaussian N(m, S) and a unit Gaussian N(0, I)"""
    key = jax.random.PRNGKey(random.randint(a=0, b=999))
    m = jax.random.uniform(key=key, shape=(num_data, dimension), minval=0.1, maxval=4.)
    log_S = jax.random.uniform(key=key, shape=(num_data, dimension), minval=0.1, maxval=.9)
...
def test_scaled_sum_squared_loss(num_data, dimension):
    ...
    expected_val = jnp.sum(0.5 * (y - y_reconstruction)**2/vae_variance, axis=-1)
...
def test_pixel_sum_loss(num_data, dimension):
    """Test the pixel sum loss"""
    key = jax.random.PRNGKey(random.randint(a=0, b=999))
    y = jax.random.uniform(key=key, shape=(num_data, dimension, dimension, 1), minval=0.1, maxval=4.)
    key, _ = jax.random.split(key)
